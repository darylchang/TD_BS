...
def train(numAgents=3, numGames=30):
	...
	for i in range(numAgents):
		...
		playerWArray.append(w)

	agents = [agent.ReflexAgent(i, logLinearEvaluation, playerWArray[i]) for i in range(numAgents)]
	...
	finalWeight = [0 for i in range(len(playerWArray[0]))]
	for i in range(len(finalWeight)):
		t = 0.0
		for weight in playerWArray:
			t += weight[i]
		finalWeight[i] = t / len(playerWArray) 
